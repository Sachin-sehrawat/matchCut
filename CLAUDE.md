...
- Within `screen === 'app'`, `tab` controls which of the five tab-bar screens is shown (`home`/`reels`/`matches`/`friends`/`profile`).
- `reels` is a TikTok-style vertical trailer feed (`ReelsScreen`/`ReelItem` in `App.jsx`), self-contained and independent of the swipe deck — it's watch-only browsing, not another way to swipe. It shows TMDB's global trending list (`GET /api/movies/trending`, not filtered by onboarding genres/language/region or swipe history), scrolls with CSS `scroll-snap`, and lazily fetches each movie's trailer (`GET /api/movies/:id/trailer`, the same endpoint Discover uses) only once that reel is the one on screen (tracked via `IntersectionObserver`), plus paginates in more trending movies as the user nears the end of the loaded list. Since an `<iframe>` can't use `object-fit`, each `ReelItem` measures its own container with a `ResizeObserver` (`useCoverSize`) to size the embedded YouTube player in real pixels — CSS `vw`/`vh` units don't work here because the reel container isn't reliably the full browser viewport (e.g. on desktop the phone frame is narrower than the window).
