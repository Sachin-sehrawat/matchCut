...
- `src/routes/auth.js` (`/api/auth/register`, `/api/auth/login`), `src/routes/me.js` (`/api/me`, `PATCH /api/me/preferences`), `src/routes/movies.js` (`/api/movies`, `/api/movies/:id/trailer`), `src/routes/swipes.js` (`POST /api/swipes` — records a swipe, updates genre preferences, returns `{matched}`), `src/routes/friends.js` (`/api/friends`, `/api/friends/invite` — accepts a username or an email, `/api/friends/:id/accept`, `/api/friends/:id/partner`), `src/routes/admin.js` (`POST /api/admin/export`, founder-only — see below).
- `db/schema.sql` — `users` (includes a `role` column, `'member'` | `'founder'`), `movies` (TMDB cache), `swipes`, `friendships` (one row per direction, `status` of `pending`/`connected`/`partner`), `genre_preferences` (per-user, per-genre `likes`/`dislikes` tally — see `src/preferences.js`).
